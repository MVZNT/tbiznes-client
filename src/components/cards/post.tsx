...
const PostCard = ({title, cover_image, descr}: PostCardProps) => {
    return (
        <div className={"flex flex-col gap-2 rounded-t-2xl"}>
            ...
            >
                <span
                    className={"bg-white bg-opacity-90 text-black px-3 py-1 rounded-full text-sm max-lg:text-[10px] max-lg:leading-4 font-bold"}>
                    {title}
                </span>
            ...
            <span className={"font-bold max-lg:text-sm"}>$40 000</span>
            <span className={"font-pragmaticaBold max-lg:text-xs"}>
                {descr?.length < 50 ? descr : `${descr.substring(0, 49)}...`}
            </span>

            <div className={"flex flex-col gap-1 "}>
                <span
                    className={"font-bold text-[#646464] text-sm max-lg:text-xs"}
                >
                    Ташкент, Мирзо-Улугбекский район
                </span>

                <div className={"flex max-lg:flex-col max-lg:gap-1 gap-2 text-sm max-lg:text-xs text-[#646464]"}>
